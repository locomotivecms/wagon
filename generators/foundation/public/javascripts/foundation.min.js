...
}else{if(typeof Zepto==="function"){libFuncName=Zepto;}else{throw new TypeError();}}}(function(c,b,a,d){if(!Array.prototype.filter){Array.prototype.filter=function(f){if(this==null){throw new TypeError();
...
};b.Foundation={name:"Foundation",version:"4.1.5",cache:{},init:function(o,l,e,p,f,h){var j,m=[o,e,p,f],k=[],h=h||false;if(h){this.nc=h;}this.rtl=/rtl/i.test(c("html").attr("dir"));
this.scope=o||this.scope;if(l&&typeof l==="string"){if(/off/i.test(l)){return this.off();}j=l.split(" ");if(j.length>0){for(var g=j.length-1;g>=0;g--){k.push(this.init_lib(j[g],m));
}}}else{for(var n in this.libs){k.push(this.init_lib(n,m));}}if(typeof l==="function"){m.unshift(l);}return this.response_obj(k,m);},response_obj:function(f,g){for(var h=0,e=g.length;
h<e;h++){if(typeof g[h]==="function"){return g[h]({errors:f.filter(function(i){if(typeof i==="string"){return i;}})});}}return f;},init_lib:function(f,e){return this.trap(function(){if(this.libs.hasOwnProperty(f)){this.patch(this.libs[f]);
return this.libs[f].init.apply(this.libs[f],e);}else{return function(){};}}.bind(this),f);},trap:function(f,h){if(!this.nc){try{return f();}catch(g){return this.error({name:h,message:"could not be initialized",more:g.name+" "+g.message});
